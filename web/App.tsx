...
function currentRoute(){const params=new URL(location.href).searchParams;return {session:params.get("session"),path:params.get("path")};}
function homeURL(){const url=new URL(location.href);url.searchParams.delete("session");url.searchParams.delete("path");return url;}
function Workspace({host,token}:{host:string;token:string}) {
  ...
  const view=route.session?"live":"table";
  ...
  function navigate(next?:FileRow) {
    const url=new URL(location.href);url.searchParams.delete("session");url.searchParams.delete("path");
  ...
  return <>
    ...
    <main id="workspace" data-view={view} className="grid items-start gap-6 xl:min-h-0 xl:flex-1 xl:items-stretch xl:grid-cols-[minmax(0,1fr)_420px]">
      <SessionList snapshot={backend.snapshot} selected={file?.path??""} onSelect={navigate} paused={backend.paused} busy={backend.busy} onPause={backend.pause} onRefresh={()=>void backend.refresh()} table={true} request={backend.request}/>
...
export function App() {
  ...
  const [theme,setTheme]=useState(()=>{const value=saved(themeKey);return value==="vangogh"||value==="paper"?value:"dark";});
