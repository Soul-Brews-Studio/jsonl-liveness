...
import { rowTime, classes, eventLabel, fullID, lastWrite, messageRevision, size, type FileRow } from "../model";
...
export function SessionList({snapshot,selected,onSelect,paused,busy,onPause,onRefresh,table,request}:SessionListProps) {
  ...
  return <section className="flex min-h-0 min-w-0 flex-col xl:h-full" aria-label="Sessions">
    ...
    <div className="my-3 flex flex-wrap items-center gap-2"><button id="pause" aria-pressed={paused} onClick={onPause}>{paused?"Resume":"Pause"}</button><button id="refresh" disabled={busy} onClick={onRefresh}>Refresh</button><label className="timeline-checkbox-label"><input className="timeline-checkbox" type="checkbox" checked={showIDs} onChange={event=>setShowIDs(event.target.checked)}/>Show IDs</label>{table&&<label className="ml-2 flex items-center gap-2 text-xs text-muted"><input className="timeline-checkbox" type="checkbox" checked={showProject} onChange={event=>setShowProject(event.target.checked)}/>Show project path</label>}</div>
    <div id="session-table-scroll" className="max-h-[65vh] min-h-0 overflow-auto xl:max-h-none xl:flex-1 rounded-lg border border-line">
      ...
      <tbody id="rows">{files.slice(start,start+50).map(file=><tr key={file.path} data-selected={selected===file.path} data-new={fresh.has(file.path)||undefined} className="session-row cursor-pointer border-t border-line" onClick={()=>onSelect(file)}>
        <td className="p-3">
          ...
          {selected===file.path&&<span className="mt-1 block text-[11px] text-accent">Selected</span>}
        ...
        <td className="p-3 break-words">{file.name||"Unnamed"}</td>
        <td className="p-3"><span className="whitespace-nowrap">{file.tier}</span><span className="mt-1 block text-xs text-muted">{fresh.has(file.path)&&<span className="arrival-badge text-accent">New · </span>}{openLabel(file)}</span></td>
        {showIDs&&<><td className="p-3 whitespace-nowrap font-mono text-xs" title={sessionIdentity(file).sessionId??undefined}>{sessionIdentity(file).sessionId?.slice(0,8)??"Unknown"}</td>
        <td className="p-3 whitespace-nowrap font-mono text-xs" title={sessionIdentity(file).agentId??undefined}>{sessionIdentity(file).agentId?.slice(0,14)??"—"}</td></>}
        {table&&<>{showProject&&<td className="px-2 text-muted">{file.projectSource==="cwd"?"Recorded cwd: ":"Estimated: "}{file.project}</td>}<td className={`freshness ${file.class} px-2 capitalize`}>{file.class}</td><td className="px-2 text-muted">{eventLabel(file)}</td><td className="message-cell px-3"><MessagePreview now={Date.parse(snapshot!.scannedAt)} summary={file.message} path={file.path} revision={file.revision??`${file.size}:${lastWrite(file,snapshot!.scannedAt)}`} request={request}/></td><td className="px-2 whitespace-nowrap"><time dateTime={lastWrite(file,snapshot!.scannedAt)}>{rowTime(lastWrite(file,snapshot!.scannedAt),Date.parse(snapshot!.scannedAt))}</time></td><td className="p-3 text-right font-mono">{size(file.size)}</td></>}
      </tr>)}</tbody></table>
    </div>
