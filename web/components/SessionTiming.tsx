...
export function SessionTiming({detail,updated,loading}:{detail?:SessionDetail;updated:string;loading:boolean}) {
  return <section aria-label="Session timing" className="my-5 border-y border-line py-4">
    <dl className="grid gap-4 sm:grid-cols-3">
      ...
      <div><dt className="text-xs text-muted">File updated</dt><dd id="detail-updated" className="mt-1 text-sm">{localTime(detail?.lastUpdatedAt||updated)}</dd></div>
    </dl>
    ...
    <p className="mt-3 text-[11px] leading-relaxed text-muted">Start: first timestamp in the head. End: explicit session-end only, if visible. File updated: filesystem write time, not message time. Local time.</p>
  </section>;
}
