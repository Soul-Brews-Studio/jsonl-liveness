...
import {recentTime,rowTime,localTime} from "./model";
import {createElement} from "react";
import {renderToStaticMarkup} from "react-dom/server";
import {MessagePreview} from "./components/MessagePreview";

test("row timestamps switch from relative to absolute at exactly five minutes",()=>{
  ...
  expect(rowTime(at(300000),now)).toBe(localTime(at(300000)));
  expect(recentTime(at(-1000),now)).toBe(false);
  expect(rowTime(null,now)).toBe("Not recorded");
  expect(rowTime("invalid",now)).toBe("Not recorded");
});
test("message preview keeps exact time in tooltip but recent visible text is relative",()=>{
  ...
  expect(html).toContain('title="'+localTime(timestamp)+'"');
});
